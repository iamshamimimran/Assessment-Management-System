...
if (!empty($_POST["classid"])) {
  ...
  if (!is_numeric($cid)) {
    ...
    $stmt = $dbh->prepare("SELECT RollId,StudentId FROM tblstudents WHERE ClassId= :id order by StudentName");
    $stmt->execute(array(':id' => $cid));
?><option value="">Select Student </option><?php
                                            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                            ?>
      <option value="<?php echo htmlentities($row['StudentId']); ?>"><?php echo htmlentities($row['RollId']); ?></option>
    <?php
                                            }
                                          }
                                        }
